...
public class HandAnimator : MonoBehaviour
{
    ...
    private void Awake()
    {
        Debug.Log("Parent: " + gameObject.transform.parent.gameObject.GetComponent<ActionBasedController>());
        // animator =
    ...
    private void Update()
    {
        //Debug.Log("Testingaskjdaisjdij" + controller.selectAction.action.ReadValue<float>());

        if (controller.selectAction.action.ReadValue<float>() == 1 && controller.selectAction.action.ToString().Contains("grip"))
        {
            //Debug.Log("Grip " + controller.selectAction.action);
            //Debug.Log("Grip " + controller.selectAction.action.ToString().Contains("Right"));

            grip_state = Mathf.Clamp(grip_state + 1 / anim_frames, 0f, 1);
            m_animator.SetFloat(m_animParamIndexFlex, grip_state);
        }
        else if (controller.uiPressAction.action.ReadValue<float>() == 1 && controller.uiPressAction.action.ToString().Contains("trigger"))
        {
            //Debug.Log("Trigger " + controller.uiPressAction.action);
            //Debug.Log("Trigger " + controller.uiPressAction.action.ToString().Contains("Right"));

            trigger_state = Mathf.Clamp(trigger_state + 1 / anim_frames, 0f, 1);
            m_animator.SetFloat("Pinch", trigger_state);

        }
        else if (controller.activateAction.action.ReadValue<float>() != 0)
        {
            Debug.Log("Huh " + controller.activateAction.action.ReadValue<float>() + " " + controller.activateAction.action);
        }
        else
        {
            grip_state = Mathf.Clamp(grip_state - 1 / anim_frames, 0, 1f);
            m_animator.SetFloat(m_animParamIndexFlex, grip_state);

            trigger_state = Mathf.Clamp(trigger_state - 1 / anim_frames, 0, 1f);
            m_animator.SetFloat("Pinch", trigger_state);
