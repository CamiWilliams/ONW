...
/**
 * This class handles gesture detection for hand-tracking.
 * Edited from https://github.com/jojods1125/VRGestureDetection
 * 
 * From the scene it takes in the center eye anchor, the left hand and the right hand.
 */
public class HandManager : MonoBehaviour
{
    /** Center eye from OVRCameraRig */
    public GameObject centerEyeAnchor;

    /** Left hand from OVRCameraRig */
    public OVRHand leftOVRHandPrefab;

    /** Right hand from OVRCameraRig */
    public OVRHand rightOVRHandPrefab;

    // Left hand skeleton for bone info
    private OVRSkeleton leftSkele;

    // Right hand skeleton for bone info
    private OVRSkeleton rightSkele;

    // Primary hand as set by user in the GameManager class, with a default as Right
    private OVRHand primaryHand;

    // Secondary hand as set by user in the GameManager class, with a default as Left
    private OVRHand secondaryHand;

    // Primary hand skeleton for bone info
    private OVRSkeleton primarySkele;

    // Secondary hand skeleton for bone info
    ...
    // Current gesture being detected by a GestureLibrary
    private string currentGesture;

    // Number of gesture libraries in the project
    private int libraryCount = 0;

    // Number of libraries reporting no gesture on update()
    private int emptyCount = 0;

    // Number of libraries reporting a gesture on update()
    private int trackedCount = 0;

    // List of existing snapshot gesture names
    private string[] snapshots;

    // Dictionary of multi-part gesture snapshots and the time remaining for the next snapshot
    ...
    {
        // Palm facing the player
        Palm_Back,
        // Palm facing between the player and away from the player
        Palm_Mid_Fr,
        // Palm facing away from the player
        Palm_Front,


        // Palm facing to the outside (right on right hand, left on left hand)
        Palm_Out,
        // Palm facing between the outside and inside
        Palm_Mid_In,
        // Palm facing to the inside (left on right hand, right on left hand)
        Palm_In,


        // Palm facing upward
        Palm_Up,
        // Palm facing between upward and downward
        Palm_Mid_Dw,
        ...
        Knuckles_Down,
        // Knuckles facing between upward and downward
        Knuckles_Mid,
        // Knuckles facing upward
        Knuckles_Up,

        // Knuckles facing to the inside (left on right hand, right on left hand)
        Knuckles_In,

        // Thumb facing downward
        Thumb_Down,
        // Thumb facing between upward and downward
        Thumb_Mid,
        // Thumb facing upward
        Thumb_Up,
     ...
     * Primary is right and secondary is left by default.
     * @param the hand being set to primary
     */
     ...
     * @returns The primary hand as set by the program; defaults to the right hand
     */
     ...
     * @returns The secondary hand as set by the program; defaults to the left hand
     */
     ...
     * @returns HandType.Left if left hand, HandType.Right if right hand
     */
     ...
     * @returns The primary hand skeleton as set by the program; defaults to the right hand
     */
     ...
     * @returns The secondary hand skeleton as set by the program; defaults to the left hand
     */
     ...
     * @returns The left hand skeleton
     */
     ...
     * @returns The current gesture written as a string
     */
     ...
     * @returns Array of strings representing snapshot gestures
     */
     ...
     * @returns Dictionary of gestures as strings to time as float
     */
     ...
     * @param newGesture The gesture being detected
     */
     ...
     * @param boneId The numeric bone ID, using Oculus' bone data
     * 
     * @returns The bone name as a string
     */
     ...
     *     Positive vector is (forward, right, up)
     * @returns Hand location as a vector in the format (x, y, z), with origin roughly in the center of the player's eyesight
     */
...
public class Finger
{
    ...
    /**
     * Determines the FingerShape being used by the Finger if FingerType is a thumb. 
     * @returns The current active FingerShape for the thumb
     */
